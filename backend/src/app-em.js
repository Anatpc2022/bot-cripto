    ...
    try {
      ...
      if (order.status === "FILLED") {
        ...
        if (updatedOrder.automationId)
          riberBot.updateMemory(
            order.symbol,
            `${indexKeys.AUTO_ORDER}_${updatedOrder.automationId}`,
            null,
            updatedOrder.get({ plain: true }),
          );
      }
...
async function init(userId, wssInstance) {
  ...
  await Promise.all(
    lastAutomationOrders.map((order) =>
      riberBot.updateMemory(
        order.symbol,
        `${indexKeys.AUTO_ORDER}_${order.automationId}`,
        null,
        order.get({ plain: true }),
        false,
      ),
  ...
  userMonitors
    ...
    .map((monitor, i) => setTimeout(() => startChartMonitor(monitor), 250 * i));
