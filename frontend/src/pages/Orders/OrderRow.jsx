...
export default function OrderRow(props) {
    ...
    function getStatus(status) {
        let colorClass;
        switch (status) {
            case ORDER_STATUS.FILLED: colorClass = "bg-success"; break;
            case ORDER_STATUS.PARTIALLY_FILLED: colorClass = "bg-info"; break;
            case ORDER_STATUS.EXPIRED:
            case ORDER_STATUS.CANCELED:
            case ORDER_STATUS.REJECTED: colorClass = "bg-danger"; break;
            default: colorClass = "bg-warning"; break;
        }
        return (<span className={"py-1 badge " + colorClass}>{status.split("_")[0]}</span>)
    }
    ...
    return (
        <tr>
            <td>
                {
                    props.data.automationId
                        ...
                        : <svg className="icon icon-xs me-2" fill="currentColor" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            ...
            <td><span className="fw-normal">{getStatus(props.data.status)}</span></td>
            <td>
                <a className="btn btn-info btn-xs" href={"/orders/view/" + props.data.id}>
                    <svg className="icon icon-xs" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
